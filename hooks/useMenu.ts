...
const useMenu = () => {
  ...
  function getSecondLevelByMenu(menuName: string) {
    return MENU_DATA.find((menu) => menu.name === menuName).subCategories.map(
      (sub) => sub.name
    );
  }

  function getThirdLevelByMenu(
    firstLevelMenu: string,
    secondLevelMenu: string
  ) {
    return MENU_DATA.find(
      (menu) => menu.name === firstLevelMenu
    ).subCategories.find((sub) => sub.name === secondLevelMenu).subCategories;
  }
